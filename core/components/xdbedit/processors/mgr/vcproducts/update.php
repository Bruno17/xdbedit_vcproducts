...
switch ($scriptProperties['task']) {
    ...
    default:
		...
		$modx->xdbedit->shopid=$scriptProperties['shopid'];
        ...
        */
        ...
        if ($scriptProperties['object_id'] == 'neu'){
			...
			$postvalues['shopid']=$scriptProperties['shopid'];
        }
        else{
            $object = $modx->getObject($classname, $scriptProperties['object_id']);
		...
		//handle published
		if ($postvalues['active']=='1'){
		    $pub=$object->get('active');
			if (empty($pub)){
			...
			$unpub=strtotime($unpub);
		    if($unpub<$now){
		        $postvalues['unpublishdate']='';
		    }            	
            }			
			
		}
		if ($postvalues['active']=='0'){
		    $pub=$object->get('publishdate');
            if (!empty($pub)){
			$pub=strtotime($pub);
			//echo (time()).' ';
			if($pub<$now){
		        $postvalues['publishdate']='';
		    }            	
            }
		...
		if (empty($postvalues['ow_alias'])) {
			...
			if (empty($postvalues['alias'])) {
			    $postvalues['alias'] = $resource->cleanAlias($postvalues['name']);
            }
		    else{
